...
-- 1. BATCH_JOB_INSTANCE  (배치 작업의 고유 인스턴스 정보)
...
-- 2. BATCH_JOB_EXECUTION (배치 작업 실행 정보)
...
-- 3. BATCH_JOB_EXECUTION_PARAMS(작업 실행 시 전달된 파라미터)
...
-- 4. BATCH_STEP_EXECUTION(각 스텝의 실행 정보)
...
-- 5. BATCH_STEP_EXECUTION_CONTEXT(스텝 실행 컨텍스트)
...
-- 6. BATCH_JOB_EXECUTION_CONTEXT(작업 실행 컨텍스트)
...
-- 2-1. settlements 테이블: IN_PROGRESS 상태 + 날짜 최적화
