...
/**
 * 정산 상태 갱신용 ItemReader
 * 배송완료 후 7일이 경과한 PENDING 정산들을 청크 단위로 읽어옴
 */
@Slf4j
public class SettlementUpdateItemReader implements ItemReader<SettlementBatchItem> {
    ...
    public SettlementBatchItem read() throws Exception, UnexpectedInputException,
        ...
        if (currentChunkIterator == null || !currentChunkIterator.hasNext()) {
            ...
            if (!hasMoreData) {
                log.info("정산 상태 갱신 대상 데이터 읽기 완료 - 총 처리된 offset: {}", currentOffset);
                return null;
            }
            ...
            if (currentChunkIterator == null || !currentChunkIterator.hasNext()) {
                log.info("정산 상태 갱신 대상 데이터가 더 이상 없습니다 - offset: {}", currentOffset);
                return null;
            }
        }
        ...
        log.debug("정산 상태 갱신 아이템 읽기 - settlementId: {}, orderNumber: {}, deliveredAt: {}",
                updateItem.getSettlementId(), updateItem.getOrderNumber(), updateItem.getDeliveredAt());
    ...
    private void loadNextChunk() {
        try {
            log.debug("정산 상태 갱신 대상 데이터 로드 시작 - offset: {}, limit: {}", currentOffset, chunkSize);

            List<SettlementBatchItem> chunk = settlementChunkMapper
                    .findPendingSettlementsReadyForProgress(currentOffset, chunkSize);

            if (chunk.isEmpty()) {
                log.info("정산 상태 갱신 대상 데이터 없음 - offset: {}", currentOffset);
                hasMoreData = false;
                currentChunkIterator = null;
                return;
            }

            log.info("정산 상태 갱신 대상 데이터 로드 완료 - offset: {}, 로드된 건수: {}", currentOffset, chunk.size());
            ...
            log.error("정산 상태 갱신 대상 데이터 로드 실패 - offset: {}", currentOffset, e);
