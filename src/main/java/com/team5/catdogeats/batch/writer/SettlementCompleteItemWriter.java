...
/**
 * 정산 완료용 ItemWriter
 * IN_PROGRESS 상태의 정산을 COMPLETED로 변경
 */
@Slf4j
@RequiredArgsConstructor
public class SettlementCompleteItemWriter implements ItemWriter<SettlementBatchItem> {
    ...
    public void write(Chunk<? extends SettlementBatchItem> chunk) throws Exception {
        if (chunk.isEmpty()) {
            log.debug("정산 완료 Writer - 처리할 데이터 없음");
            return;
        }

        log.info("정산 완료 Writer 시작 - 처리 대상 건수: {}", chunk.size());
        ...
        for (SettlementBatchItem item : chunk) {
            try {
                ...
                if (!item.isValidForComplete()) {
                    log.warn("정산 완료 데이터 유효성 검증 실패 - item: {}", item);
                    failCount++;
                    continue;
                }

                // 정산 상태를 COMPLETED로 변경
                int updateCount = settlementChunkMapper.updateToCompleted(item.getSettlementId());

                if (updateCount == 1) {
                    successCount++;
                    log.debug("정산 완료 처리 완료 - settlementId: {}, orderNumber: {}, settlementAmount: {}",
                            item.getSettlementId(), item.getOrderNumber(), item.getSettlementAmount());
                } else {
                    // 업데이트된 행이 없는 경우 (이미 상태가 변경되었거나 데이터가 없음)
                    noUpdateCount++;
                    log.warn("정산 완료 처리 실패 - 대상 데이터 없음. settlementId: {}, orderNumber: {}",
                            item.getSettlementId(), item.getOrderNumber());
                }

            } catch (Exception e) {
                log.error("정산 완료 처리 실패 - item: {}", item, e);
                failCount++;
        ...
        log.info("정산 완료 Writer 완료 - 성공: {}, 업데이트없음: {}, 실패: {}, 총 처리: {}",
        ...
        if (failCount > 0 || noUpdateCount > 0) {
            log.warn("정산 완료 처리 중 일부 실패/미처리 발생 - 실패: {}, 미처리: {}, 총: {}",
