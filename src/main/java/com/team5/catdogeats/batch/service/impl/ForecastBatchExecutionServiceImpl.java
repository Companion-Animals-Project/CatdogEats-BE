...
public class ForecastBatchExecutionServiceImpl implements ForecastBatchExecutionService {

    private final JobLauncher jobLauncher;
    private final ForecastBatchConcurrencyService forecastBatchConcurrencyServiceimpl;
    ...
    private BatchExecutionResult executeBatchWithLock(String batchName, String executionId,
        ...
        // 1. 락 획득 시도
        boolean lockAcquired = forecastBatchConcurrencyServiceimpl.tryAcquireLock(batchName, executionId);
        if (!lockAcquired) {
        ...
        try {
            ...
            if (BatchStatus.COMPLETED.equals(batchStatus)) {
                forecastBatchConcurrencyServiceimpl.releaseLockAsCompleted(batchName);
                return BatchExecutionResult.success(jobExecution);
            } else {
                forecastBatchConcurrencyServiceimpl.releaseLockAsFailed(batchName);
